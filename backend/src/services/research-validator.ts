...
interface ValidationViolation {
  term: string;
  issue: 'translated' | 'transliterated' | 'missing';
  expected: string;
  found?: string;
  line?: number;
...
export async function validateTranslation(
  ...
  const preservedCorrectly: string[] = [];
  ...
  if (violations.length > 0) {
    suggestions.push(
      `Review ${violations.length} violation(s) and ensure technical terms remain in English`
    );
