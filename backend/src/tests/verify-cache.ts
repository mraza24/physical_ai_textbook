...
async function verifyCacheService() {
  ...
  const key1 = generateCacheKey('/docs/module1/intro.md', profile1, 'personalize');
  const key2 = generateCacheKey('/docs/module1/intro.md', profile2, 'personalize');
  const key3 = generateCacheKey('/docs/module1/intro.md', profile3, 'personalize');
  ...
  const testCacheKey = generateCacheKey(
    '/docs/test/chapter.md',
    { software_background: 'Intermediate', hardware_experience: 'Basic' },
    'personalize'
  );
  ...
  try {
    await setCacheEntry(testCacheKey, testContent, testMetadata);
  ...
  try {
    const cached = await getCacheEntry(testCacheKey);
