...
router.post('/signup', async (req, res) => {
    try {
        const { email, password, software_background, hardware_experience, language_preference = 'English', python_level, ros2_level, gpu_available, hardware_tier, primary_goal, } = req.body;
        ...
        const signupResult = await config_1.auth.api.signUpEmail({
            body: {
        ...
        try {
            await connection_1.db.insert(schema_1.userProfile).values({
                user_id: userId,
                software_background,
                hardware_experience,
                language_preference,
...
router.put('/profile', async (req, res) => {
    try {
        ...
        const { software_background, hardware_experience, language_preference, python_level, ros2_level, gpu_available, hardware_tier, primary_goal, } = req.body;
        ...
        if (software_background !== undefined)
            updateData.software_background = software_background;
        if (hardware_experience !== undefined)
            updateData.hardware_experience = hardware_experience;
        if (language_preference !== undefined)
            updateData.language_preference = language_preference;
        if (python_level !== undefined)
            updateData.python_level = python_level;
        if (ros2_level !== undefined)
            updateData.ros2_level = ros2_level;
        if (gpu_available !== undefined)
            updateData.gpu_available = gpu_available;
        if (hardware_tier !== undefined)
            updateData.hardware_tier = hardware_tier;
        if (primary_goal !== undefined)
            updateData.primary_goal = primary_goal;
        await connection_1.db
            .update(schema_1.userProfile)
            .set(updateData)
            .where((0, drizzle_orm_1.eq)(schema_1.userProfile.user_id, userId));
        // Fetch updated profile
        const [updatedProfile] = await connection_1.db
            .select()
