...
router.post('/urdu', async (req, res) => {
    ...
    try {
        ...
        const { chapterPath, content, technicalTerms = [] } = req.body;
        ...
        const allTechnicalTerms = Array.from(new Set([...DEFAULT_TECHNICAL_TERMS, ...technicalTerms]));
        ...
        const { transformed, warnings } = await (0, markdown_processor_1.safeTransform)(content, async (textOnly) => {
            ...
            return await (0, llm_client_1.translateToUrdu)(textOnly, allTechnicalTerms, chapterPath);
        });
 ...
 * }
 */
router.get('/urdu/status', async (req, res) => {
    try {
        const userId = req.user?.id;
