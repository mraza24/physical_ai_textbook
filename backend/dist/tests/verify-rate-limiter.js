...
async function verifyRateLimiter() {
    ...
    app1.use((0, rate_limiter_1.createUserRateLimiter)(10, 1));
    app1.get('/test', (req, res) => res.json({ success: true }));
    let successCount = 0;
    let failureCount = 0;
    // Send 11 requests from same IP
    for (let i = 1; i <= 11; i++) {
        const res = await (0, supertest_1.default)(app1).get('/test');
        if (res.status === 200) {
            successCount++;
    ...
    app2.use((0, rate_limiter_1.createTransformationRateLimiter)(5, 1));
    app2.post('/transform', (req, res) => res.json({ transformed: true }));
    let transformSuccess = 0;
    let transformFailure = 0;
    // Send 6 requests
    for (let i = 1; i <= 6; i++) {
        const res = await (0, supertest_1.default)(app2).post('/transform');
        if (res.status === 200) {
            transformSuccess++;
            console.log(`  Request ${i}: ✅ 200 OK`);
        }
    ...
    app3.use((0, rate_limiter_1.createAuthRateLimiter)());
    app3.post('/auth/login', (req, res) => res.json({ token: 'fake-jwt' }));
    let authSuccess = 0;
    let authFailure = 0;
    // Send 6 login attempts
    for (let i = 1; i <= 6; i++) {
        const res = await (0, supertest_1.default)(app3).post('/auth/login');
        if (res.status === 200) {
            authSuccess++;
