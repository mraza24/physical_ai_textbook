...
async function verifyLLMClient() {
    ...
    try {
        const connected = await (0, llm_client_1.testLLMConnection)();
        if (connected) {
...
ROS2 provides publish-subscribe messaging for distributed systems.`;
    try {
        ...
        const personalized = await (0, llm_client_1.personalizeContent)(testChapter, 'Beginner', 'None');
...
\`\`\``;
    try {
        ...
        const translated = await (0, llm_client_1.translateToUrdu)(shortChapter, ['ROS2', 'framework']);
...
\`\`\``;
    try {
        ...
        const validation = await (0, llm_client_1.validateChapter)(chapterToValidate);
